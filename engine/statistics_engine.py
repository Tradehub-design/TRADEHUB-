class StatisticsEngine:
    ...
    def summary(trades):
        ...
        return {
            ...
            "net_profit": round(trades["net_profit"].sum(), 2),
            "gross_profit": round(gross_profit, 2),
            "gross_loss": round(gross_loss, 2),
            "profit_factor": profit_factor,
            "average_trade": round(trades["net_profit"].mean(), 2) if len(trades) else 0,
            "average_win": round(wins["net_profit"].mean(), 2) if len(wins) else 0,
            "average_loss": round(losses["net_profit"].mean(), 2) if len(losses) else 0,
        }
